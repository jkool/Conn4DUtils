...
/**
 * 
 * Adds a unique identifier to each path in the database using a combination of
 * source name and id. Also performs clean-up and indexing.
 * 
...
public class DBAddPathID extends DBProcessTableSet {
	...
	public void process(String name) throws SQLException {
		if (this.fieldExists(name, fieldName)) {
			System.out.println(name + " already contains field '" + fieldName
					+ "'.  Skipping.");
		} else {
			...
			System.out.printf("\t%-50s%-30s", "Finding distinct values",
			...
			System.out.printf("\t%-50s%-30s",
					"Ordering and numbering distinct values",
					new Date(System.currentTimeMillis()));
			t1 = System.currentTimeMillis();
			stmt.execute("CREATE TABLE "
					+ name
					+ "_key AS SELECT row_number() over (ORDER BY source,id) AS "
					+ fieldName + ", source, id FROM " + name + "_rel");
			t2 = System.currentTimeMillis();
			System.out.printf("%-15s%n",
					"(" + TimeConvert.millisToString(t2 - t1) + ")");
			...
			System.out.printf("\t%-50s%-30s",
					"Generating indexes on distinct values",
					new Date(System.currentTimeMillis()));
			t1 = System.currentTimeMillis();
			...
			System.out.printf("\t%-50s%-30s",
					"Joining unique path ID to the original table", new Date(
							System.currentTimeMillis()));
			stmt.execute("DROP TABLE IF EXISTS " + name + "_tmp");
			t1 = System.currentTimeMillis();
			stmt.execute("CREATE TABLE " + name + "_tmp AS SELECT t1.*,t2."
					+ fieldName + " FROM " + name + " t1 INNER JOIN " + name
			...
			System.out.printf("\t%-50s%-30s",
					...
					new Date(System.currentTimeMillis()));
			t1 = System.currentTimeMillis();
			stmt.execute("DELETE FROM " + name + "_tmp t1 USING " + name
					+ "_tmp t2 WHERE t1." + fieldName + "=t2." + fieldName
					+ " AND t1.time_=t2.time_ AND t1.key<t2.key");
			t2 = System.currentTimeMillis();
			System.out.printf("%-15s%n",
					"(" + TimeConvert.millisToString(t2 - t1) + ")");
			...
			System.out.printf("\t%-50s%-30s",
					"Replacing original table with new table",
					new Date(System.currentTimeMillis()));
			t1 = System.currentTimeMillis();
			if (!tableExists(name + "_tmp")) {
				...
				// Add geometry if not already created

				if (!fieldExists(name, "geom")) {

					System.out.printf("\t%-50s%-30s", "Adding geometry",
							new Date(System.currentTimeMillis()));
					t1 = System.currentTimeMillis();
					stmt.execute("SELECT AddGeometryColumn('public','" + name
							+ "','geom',4326,'POINT',3)");
					stmt.execute("UPDATE "
							+ name
							+ " SET geom = ST_SetSRID(ST_MakePoint(lon, lat, depth),4326);");
					t2 = System.currentTimeMillis();
					System.out.printf("%-15s%n",
							"(" + TimeConvert.millisToString(t2 - t1) + ")");
				}
				...
				System.out.printf("\t%-50s%-30s", "Adding primary key",
						new Date(System.currentTimeMillis()));
				t1 = System.currentTimeMillis();
